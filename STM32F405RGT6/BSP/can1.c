...
/*************************************************************************
                          CAN1_Configuration
Description：Reset CAN1 Configuration to 1M Baud
*************************************************************************/
...
//If break, enter this function once per 1 ms. 如果中断开启，则1ms进入该函数一次
#define CurrTempNUM 100 //Inergraled time peiod
#define CurrLimit 13000 //Current limit is 12000 for 6025
...
void CAN1_RX0_IRQHandler(void)
{
    ...
    if (CAN_GetITStatus(CAN1,CAN_IT_FMP0)!= RESET) 
	{
        ...
        {             
             //Acquire Head Motor0x201's encoding disk value 获得云台电机0x201的码盘值                 
        }
        if(rx_message.StdId == 0x202)
        { 
             //Acquire Head Motor0x202's encoding disk value 获得云台电机0x202的码盘值           
        }		
        if(rx_message.StdId == 0x203)
        { 
             //Acquire Head Motor0x203's encoding disk value 获得云台电机0x203的码盘值  
        }
        Can1_CNT++;		
    }
}
